...
export async function getLatestOnPurposeEpisode(): Promise<Episode | null> {
  ...
  try {
    ...
    const latest = itemsData.items?.[0]?.snippet as PlaylistSnippet | undefined;
    if (!latest) return null;

    return toEpisode(latest);
  } catch (error) {
